...
public class KMeans implements OutlierMethod {
    ...
    public int NUM_CLUSTERS = 4;    // Total clusters.
...
//            System.out.println();
        ...
        int min_size_cluster_outlier = min(Cluster_size);
        ArrayList<Integer> other_sizes = new ArrayList<>();

        for (int p = 0; p < Cluster_size.size(); p++) {
            if (Cluster_size.get(p) != min_size_cluster_outlier) {
                other_sizes.add(Cluster_size.get(p));
            }
        }
...
//        System.out.println("The cluster of outlier :----");
        ...
        for (int a = 0; a < List_clusers.size(); a++) {
            ...
            if (List_clusers.get(a).size() == min_size_cluster_outlier && min_size_cluster_outlier > 0) {

                for (int b = 0; b < List_clusers.get(a).size(); b++) {
//                    System.out.println(List_clusers.get(a).get(b).toString());
...
//        System.out.println("Centroids initialized at:");
        ...
        centroids.add(new Centroid(0.0, 0.0)); // highest set.
